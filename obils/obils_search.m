       ...
       if newprsd < beta  % Inside the ellispoid
           if k ~= 1 % Move to level k-1
               ...
               if ulevel ~= 0 
                   path(ulevel:k-1) = k;
                   for j = ulevel-1 : -1 : 1
                       if path(j) < k, path(j) = k;
                       else, break;  % Note path(1:j-1) >= path(j)
                       end
                   end
               end
               % Update S
               k = k - 1;
               for j=path(k):-1:k+1, S(k,j-1)=S(k,j)-R(k,j)*z(j);end
...
% init finds the initial integer and the search direction at level_k 
...
% update finds a new integer at level k and record it if it hits
% a boundary
function [z_k,d_k,lflag_k,uflag_k] = ...
                              update(z_k,d_k,lflag_k,uflag_k,l_k,u_k)
z_k = z_k + d_k; 
if z_k == l_k, lflag_k = 1; d_k = -d_k + 1;
elseif z_k == u_k, uflag_k = 1; d_k = -d_k - 1;
elseif lflag_k == 1, d_k = 1;
elseif uflag_k == 1, d_k = -1;
else
    if d_k > 0, d_k = -d_k - 1;
    else, d_k = -d_k + 1;
    end
